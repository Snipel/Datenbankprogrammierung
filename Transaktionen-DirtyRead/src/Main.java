...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("A: Sleep Start");
		Thread.sleep(5000);
		System.out.println("A: Sleep Ende");

		rs = s.executeQuery("SELECT * FROM Konto WHERE KundenID = 2");
		rs.next();
		saldo = rs.getInt("Saldo______");
		int dispo = rs.getInt("Dispo______");
		if ((saldo + dispo) < ueberweisungsbetrag) {
			System.out.println("rollback von A");
			connect.rollback();
		} else {
			System.out.println("commit von A");
			saldo = saldo - ueberweisungsbetrag;
			rs.updateInt("Saldo______", saldo);
			rs.updateRow();
			connect.commit();
